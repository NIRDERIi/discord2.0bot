...
class Logs(commands.Cog):
    ...
    async def db_messages(self, message: discord.Message):
        if message.author.bot:
            return
        async with self.bot.pool.acquire(
            timeout=Time.BASIC_DBS_TIMEOUT()
        ) as conn:
    ...
    async def pinned_message(self, message: discord.Message):
        ...
        if not webhook:
            return
        if message.type is not discord.MessageType.pins_add:
            return
    ...
    async def member_update_logs(self, before: discord.Member, after: discord.Member):
        ...
        if before.nick != after.nick:
            ...
            if len(before.roles) > len(after.roles):
                changed_roles = list(set(before.roles) - set(after.roles))
                before_changes = f''
