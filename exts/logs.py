...
class Logs(commands.Cog):
    ...
    async def db_messages(self, message: discord.Message):
        ...
        async with self.bot.pool.acquire(timeout=Time.BASIC_DBS_TIMEOUT()) as conn:
            attachment = await message.attachments[0].read() if message.attachments else None
            await conn.execute('''INSERT INTO messages VALUES($1, $2, $3, $4, $5, $6)''', message.guild.id, message.channel.id, message.id, message.author.id, message.content, attachment)
            self.db_messages_cache[message.id] = {'guild_id': message.guild.id, 'channel_id': message.channel.id, 'author_id': message.author.id, 'message_content': message.content, 'attachment': attachment}
    ...
    async def get_db_message(self, message_id: discord.Message) -> tuple:
        if self.db_messages_cache.get(message_id):
            ...
            async with self.bot.pool.acquire(timeout=Time.BASIC_DBS_TIMEOUT()) as conn:
                data = await conn.fetch('''SELECT * FROM messages WHERE message_id = ($1)''', message_id)
                if not data:
    ...
    async def update_db_message(self, message_id: int, message_content: str):
        ...
        async with self.bot.pool.acquire(timeout=Time.BASIC_DBS_TIMEOUT()) as conn:
            await conn.execute('''UPDATE messages SET message_content = ($1) WHERE message_id = ($2)''', message_content, message_id)
    ...
    async def dispatch_channel_update_event(self, before: discord.abc.GuildChannel, after: discord.abc.GuildChannel, webhook: discord.Webhook):
    ...
    async def on_text_channel_update(self, before: discord.TextChannel, after: discord.TextChannel, webhook: discord.Webhook):
        changes = ''
        before_change = ''
        after_change = ''
        executer = 'Not found.'
        ...
        embed = discord.Embed(title='Guild channel update.', color=discord.Colour.blurple(), description=description, timestamp=datetime.datetime.utcnow())
        await webhook.send(embed=embed)
                
    async def on_voice_channel_update(self, before: discord.VoiceChannel, after: discord.VoiceChannel, webhook: discord.Webhook):
        changes = ''
        before_change = ''
        after_change = ''
        executer = 'Not found.'
        if before.category != after.category:
            ...
            changes = 'New region.'
            before_change = str(before.rtc_region) if before.rtc_region else 'Automatic.'
    ...
    async def on_stage_channel_update(self, before: discord.StageChannel, after: discord.StageChannel, webhook: discord.Webhook):
        changes = ''
        before_change = ''
        after_change = ''
        executer = 'Not found.'
    ...
    async def raw_message_delete_logs(self, payload: discord.RawMessageDeleteEvent):
        ...
        content = payload.cached_message.content if payload.cached_message else 'Couldn\'t fetch.'
        ...
        await webhook.send(embed=build_embed(title='Message delete.', description=description, timestamp=datetime.datetime.utcnow()))



    @commands.Cog.listener(name='on_raw_bulk_message_delete')
    async def raw_bulk_message_delete_logs(self, payload: discord.RawBulkMessageDeleteEvent):
        guild = self.bot.get_guild(payload.guild_id)
        webhook = await self.get_webhook(guild)
        if not webhook:
            return
        ...
        await webhook.send(embed=build_embed(title='Bulk message delete.', description=description, timestamp=datetime.datetime.utcnow()))
    ...
    async def guild_channel_delete_logs(self, channel: discord.abc.GuildChannel):
        ...
        await webhook.send(embed=build_embed(title='Guild channel delete.', description=description, timestamp=datetime.datetime.utcnow()))
    ...
    async def guild_channel_create_logs(self, channel: discord.abc.GuildChannel):
        ...
        async for entry in channel.guild.audit_logs(limit=3, action=discord.AuditLogAction.channel_create):
            if entry.target.id == channel.id:
                ...
                break
        description = f'**Name:** {channel.name}\n\n**Type:** `{channel_type}`\n\n**Excecuter:** {executer}'
        await webhook.send(embed=build_embed(title='Guild channel created.', description=description, timestamp=datetime.datetime.utcnow()))

    

    @commands.Cog.listener(name='on_guild_channel_update')
    async def guild_channel_update_logs(self, before: discord.abc.GuildChannel, after: discord.abc.GuildChannel):
