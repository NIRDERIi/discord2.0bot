...
class ErrorHandler(commands.Cog):
    ...
    async def command_error_handler(self, ctx: CustomContext, error: commands.CommandError):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            options = difflib.get_close_matches(word=ctx.invoked_with, possibilities=[command.name for command in self.bot.commands])
            if options:
                options_str = ', '.join([f'`{option}`' for option in options])
                embed.description += f'\n\n**Close matches:** {options_str}'
        elif isinstance(error, commands.MissingPermissions):
            permissions_missing = ', '.join([f'`{permission}`' for permission in error.missing_permissions])
