...
class LoggerHandler:
    ...
    def __init__(self, name = 'discord', filename ='discord.log', encoding='utf-8', mode='w', format=BASIC_FORMAT):
        self.name = name
    ...
    def _start_handler(self):
        ...
        handler = logging.FileHandler(filename=self.filename, encoding=self.encoding, mode=self.mode)
        handler.setFormatter(logging.Formatter(self.format))
        logger.addHandler(hdlr=handler)


class CustomContext(commands.Context):
    async def send_confirm(self, *args, check: typing.Callable[..., bool]=None, **kwargs):
        if not kwargs.get('view'):
            view = utility.buttons.ConfirmButtonBuild(timeout=Time.BASIC_TIMEOUT(), ctx=self, check=check)
...
class Bot(commands.Bot):
    ...
    def start_logger(self, name = 'discord', filename ='discord.log', encoding='utf-8', mode='w', format=BASIC_FORMAT):
        logger = LoggerHandler(name=name, filename=filename, encoding=encoding, mode=mode, format=format)
    ...
    def get_extensions_files(self, path: str, invalid_files: list = []) -> list:
...
async def get_prefix(bot: Bot, message: discord.Message):
    if not message.guild and message.author.id not in bot.allowed_users:
        return
    if not message.guild.id in bot.prefixes:
        async with bot.pool.acquire() as conn:
            data = await conn.fetch('''SELECT prefix FROM guilds_config WHERE guild_id = ($1)''', message.guild.id)
        if not data:
